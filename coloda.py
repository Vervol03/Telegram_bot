...
def new():
    cards = mix()
    p,b = distribution(cards)
    return {
        "cards": cards,
        "player": p,
        "bot": b,
        "trum":  cards[-1],
        "text": first_walks_text(p,b,cards[-1]),
        "move":  first_walks(p,b,cards[-1]),
        "table": [],
        "car": 0,
        "card_request": True,
        "podkidat": True,
...
def first_walks_text(player, bot, trum):
    ...
    for i in range(6):
        ...
        if bot[i][0]   ==trum[0] and ind(bot[i])   <min: min = ind(bot[i]);   chek = "bot"
    try: 
        if chek == "player": return "У вас меньше козирь!"
        else: return "У противника меньше козирь!"
    except: return "Козирів немає тому перший ходить ігрок!"

def print_inf(table, trum, cards):
    text = ",".join([i for i in table])
    if table!=[]:
        return "Стол - " +text+"\nКозирь - "+str(trum)+" Колода - "+str(len(cards))
    else: return "Козирь - "+str(trum) + " Колода - "+str(len(cards))
...
def hod_bot(table, bot_c, trum):
    if table==[]:
        ...
        text1 = "Противник ходить - "+ b
    else:
        for i in bot_c:
            for j in table:
                if i[0]!=trum[0] and i[1:]==j[1:]: b = i
        try: text1 = "Противник підкида - "+ b
